...
class WebFormAdvanced(WebsiteGenerator):
	...
	def onload(self):
		super(WebFormAdvanced, self).onload()
		if self.is_standard and not frappe.conf.developer_mode:
			self.use_meta_fields()

	def validate(self):
		super(WebFormAdvanced, self).validate()
	...
	def validate_fields(self):
		...
		for df in self.web_form_fields_advanced:
...
""")
	...
	def get_context(self, context):
		'''Build context to render the `web_form_advanced.html` template'''
	...
	def validate_mandatory(self, doc):
		...
		missing = []
		for f in self.web_form_fields_advanced:
...
def accept(web_form_advanced, data):
	'''Save the web form'''
	data = frappe._dict(json.loads(data))
	files = []
	files_to_delete = []

	web_form_advanced = frappe.get_doc("Web Form Advanced", web_form_advanced)
	if data.doctype != web_form_advanced.doc_type:
		frappe.throw(_("Invalid Request"))

	elif data.name and not web_form_advanced.allow_edit:
		frappe.throw(_("You are not allowed to update this Web Form Document"))

	frappe.flags.in_web_form_advanced = True
	...
	if doc.name:
		...
		# insert
		if web_form_advanced.login_required and frappe.session.user=="Guest":
...
@frappe.whitelist()
def delete(web_form_advanced, name):
	web_form_advanced = frappe.get_doc("Web Form Advanced", web_form_advanced)

	owner = frappe.db.get_value(web_form_advanced.doc_type, name, "owner")
	if frappe.session.user == owner and web_form_advanced.allow_delete:
		frappe.delete_doc(web_form_advanced.doc_type, name, ignore_permissions=True)
	else:
